...
namespace staticFunctional
{
    ...
    class function<R(Args...), N>
    {
        ...
        struct Invoker : InvokerBase
        {
            ...
            {
                // return R(10);
                // return invoke(*((F const*)p), std::forward<Args>(args)...);
        ...
        void _clear()
        {
            if (!empty())
            {
                _invoker_base().destroy(&_storage); // will crash the application, is it even needed?
                _empty = true;
            }
        }
        ...
        //     _empty = false;
        // }
        // --->>> replace constexpr if which does not exist in c++11 by SFINAE
